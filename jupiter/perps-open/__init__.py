...
Jupiter Protocol - Perps Open Module
Author: ECLIPSEMOON
"""
...
logger = logging.getLogger("eclipsemoon.jupiter.perps_open")

class PerpsOpen:
    """Jupiter Protocol - Perps Open implementation"""
    
    def __init__(self, client, config):
        """Initialize the Perps Open module"""
        self.client = client
        self.config = config
        ...
        logger.info(f"Initialized Jupiter Perps Open module with {self.slippage_tolerance*100}% slippage tolerance")
    ...
    async def open_position(self, 
        ...
        """
        Open a new perpetual position on Jupiter
        
        Args:
            market: The market to trade (e.g., "BTC-PERP")
        ...
        try:
            validated_inputs = await self._validate_and_normalize_inputs(
                market, direction, size, collateral, leverage, limit_price, 
        ...
        logger.info(f"Opening {direction} position of size {size} on market {market} with {leverage}x leverage")
        ...
        try:
            balance = await self._get_user_balance(wallet_address, market_info["quote_token"])
    ...
    async def open_position_with_dollar_size(self,
        ...
        if limit_price is None:
            try:
                limit_price = await self._get_market_price(market)
    ...
    async def open_scaled_position(self,
        ...
        if direction == "long":
            # For long positions, we start at current price and go down
            start_price = current_price
            end_price = current_price * (Decimal("1") - price_range_percent)
        else:
            # For short positions, we start at current price and go up
            start_price = current_price
            end_price = current_price * (Decimal("1") + price_range_percent)
            ...
            try:
                ...
                if result.get("success", False):
                    ...
                    if result.get("status") == "filled":
                        total_filled_size += size_per_order
            except Exception as e:
    ...
    async def get_available_markets(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Failed to get available markets: {str(e)}"
            logger.error(error_msg)
            return {"success": False, "error": error_msg}
    ...
    async def _validate_and_normalize_inputs(self,
        ...
        market_symbols = [m["symbol"] for m in available_markets]
        
        if market not in market_symbols:
            raise ValueError(f"Invalid market: {market}. Available markets: {', '.join(market_symbols)}")
    ...
    async def _get_market_info(self, market: str) -> Dict[str, Any]:
        ...
        # Mock market data based on market
        if "BTC" in market:
            # BTC-PERP market
            return {
                "symbol": "BTC-PERP",
    ...
    async def _get_market_price(self, market: str) -> Decimal:
        ...
        if "BTC" in market:
            ...
            return Decimal("100") + Decimal(str(math.sin(time.time() / 6) * 5))
        else:
            # Generic asset price
            base_price = Decimal("10")
            return base_price + Decimal(str(math.sin(time.time() / 5) * base_price * Decimal("0.01")))
    
    async def _get_user_balance(self, wallet_address: str, token: str) -> Decimal:
        ...
        if token == "USDC":
            ...
            return Decimal("8000")   # $8,000 USDT
        elif token == "BTC":
    ...
    async def _get_available_markets(self) -> List[Dict[str, Any]]:
        """Get available perpetual markets"""
        # This would make an actual API call to get available markets
        # For demonstration, we return mock data
        await asyncio.sleep(0.3)  # Simulate API call
        
        # Standard available markets
        return [
            {
                "symbol": "BTC-PERP",
                "base_token": "BTC",
